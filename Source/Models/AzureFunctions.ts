...
import { checkExtensionAvailable, getExtension } from "./Apis";
import {
...
export class AzureFunctions implements Component, Provisionable, Deployable {
	...
	async load(): Promise<void> {
		const componentConfig =
			await this.azureConfigFileHandler.getComponentByFolder(
	...
	async create(): Promise<void> {
		...
		if (!this.functionLanguage) {
			const picks: vscode.QuickPickItem[] = [
				...
				{
					label: AzureFunctionsLanguage.CSharpLibrary,
					description: "",
				},
			];
	...
	async updateConfigSettings(
		...
		const componentIndex =
			await this.azureConfigFileHandler.getComponentIndexById(
				type,
				this.id,
			);
		...
		if (componentIndex > -1) {
			if (!componentInfo) {
				throw new ArgumentEmptyOrNullError(
					"update config settings of IoTHub",
					"component info",
				);
			}
