...
const forEach = importLazy(() =>
	require("lodash.foreach"),
)() as typeof import("lodash.foreach");
...
export class AZ3166Device extends ArduinoDeviceBase {
	...
	get board(): Board {
		...
		if (!az3166) {
			throw new SystemResourceNotFoundError(
				"AZ3166 board",
				`board id ${AZ3166Device._boardId}`,
				"board list",
			);
		}
	...
	private async logAndSetCredentials(
		credentials: string,
		deviceSettingType: ConfigDeviceOptions,
	): Promise<void> {
		// Log Credentials
	...
	private async selectDeviceSettingType(): Promise<ConfigDeviceOptions> {
		const configSelectionItems =
			await this.getConfigDeviceSettingTypeOptions();
		...
		const deviceSettingsType: ConfigDeviceOptions = getEnumKeyByEnumValue(
			ConfigDeviceOptions,
			configSelection.label,
		);
	 ...
	 */
	private async getConfigDeviceSettingTypeOptions(): Promise<
	...
	private async getDeviceConnectionString(): Promise<string> {
		...
		if (deviceConnectionStringFromConfig) {
			...
			if (
				deviceConnectionStringAcquisitionMethodSelection.label ===
				DeviceConnectionStringAcquisitionMethods.Select
			) {
	...
	private getDPSConnectionStringOptions(): vscode.InputBoxOptions {
		const option: vscode.InputBoxOptions = {
			value: "DPSEndpoint=global.azure-devices-provisioning.net;IdScope=<Id Scope>;DeviceId=<Device Id>;SymmetricKey=<Symmetric Key>",
