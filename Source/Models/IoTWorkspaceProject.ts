...
const azureComponentConfigModule = importLazy(() =>
	require("./AzureComponentConfig"),
)();
...
export class IoTWorkspaceProject extends IoTWorkbenchProjectBase {
	 ...
	 */
	private async loadAzureComponentsByConfig(
		...
		for (const componentConfig of componentConfigs) {
			switch (componentConfig.type) {
				case ComponentType.IoTHub: {
					...
					const iothubDevice = new ioTHubDeviceModule.IoTHubDevice(
						...
						[
							{
								component: iotHub,
								type: azureComponentConfigModule.DependencyType
									.Input,
							},
						],
					);
				...
				case ComponentType.AzureFunctions: {
					...
					if (functionLocation) {
						const functionApp =
							new azureFunctionsModule.AzureFunctions(
								this.projectRootPath,
								functionLocation,
								functionPath,
								this.channel,
	 ...
	 * @param scaffoldType scaffold type
	 */
	private async loadAzureConfigAndComponents(
		scaffoldType: ScaffoldType,
	): Promise<void> {
		const azureConfigFileHandler =
			new azureComponentConfigModule.AzureConfigFileHandler(
	 ...
	 * @param scaffoldType scaffold type
	 */
	private async loadWorkspaceConfigFilePath(
		scaffoldType: ScaffoldType,
	): Promise<void> {
		await this.validateProjectRootPathExists(
			"init workspace config file path",
			scaffoldType,
		);
