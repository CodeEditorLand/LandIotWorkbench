...
const raspberryPiDeviceModule = importLazy(() =>
	require("./Models/RaspberryPiDevice"),
)();
...
export async function getTemplateFilesInfo(
	...
	const templateFilesJson = JSON.parse(
		fs.readFileSync(templateFiles, "utf8"),
	);
	...
	templateFilesJson.templateFiles.forEach((fileInfo: TemplateFileInfo) => {
		...
		templateFilesInfo.push({
			...
			overwrite:
				typeof fileInfo.overwrite !== "undefined"
					? fileInfo.overwrite
...
export async function handleExternalProject(
	...
	class Choice {
		...
		static createNewProject: vscode.MessageItem = {
			title: "Create IoT Project",
		};
	}
...
export async function getProjectConfig(
	...
	if (
		...
		(await FileUtility.fileExists(type, iotWorkbenchProjectFilePath))
	) {
...
export async function updateProjectHostTypeConfig(
	...
	if (!iotWorkbenchProjectFilePath) {
		throw new ArgumentEmptyOrNullError(
			"update project host type configuration",
			"iot workbench project file path",
		);
	}
...
export async function configExternalCMakeProjectToIoTContainerProject(
	...
	if (!projectRootPath) {
		throw new WorkspaceNotOpenError(
			"configure external CMake project to IoT container project",
		);
	}
...
export async function fetchAndExecuteTask(
	...
	if (!(await FileUtility.directoryExists(scaffoldType, deviceRootPath))) {
		throw new DirectoryNotFoundError(
			...
			`device root folder ${deviceRootPath}`,
			"",
		);
	}
	...
	try {
		...
		throw new OperationFailedError(
			`execute task to ${operationType.toLowerCase()}`,
			`${error.message}`,
			"",
		);
	}
...
export function shouldShowLandingPage(
	...
	const hasPopUp = context.globalState.get<boolean>(
		ConfigKey.hasPopUp,
		false,
	);
	return !hasPopUp;
