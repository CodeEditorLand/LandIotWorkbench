		...
		for (const key in constraintJson) {
			if (DigitalTwinGraph.isConstraintNode(constraintJson[key])) {
				...
				for (const vKey in versions) {
					this.contextVersions.set(
						versions[vKey] as string,
						parseInt(vKey.slice(1)),
					);
				}
			}
		}
	...
	private handleEdgeOfType(edge: any): void {
		...
		switch (type) {
			...
			default: {
				...
				const contextNode: ContextNode | undefined =
					this.contextNodes.get(id);
	...
	private handleEdgeOfLabel(edge: any): void {
		...
		if (!classNode.label) {
			...
			const constraintNode: ConstraintNode | undefined =
				this.constraintNodes.get(label);
