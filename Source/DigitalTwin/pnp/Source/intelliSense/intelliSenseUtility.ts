...
export class IntelliSenseUtility {
	...
	static enabled(): boolean {
		return (
			IntelliSenseUtility.graph && IntelliSenseUtility.graph.initialized()
		);
	}
	...
	static getEnums(propertyNode: PropertyNode, version: number): string[] {
		...
		for (const classNode of IntelliSenseUtility.getRangeOfPropertyByVersion(
			...
			if (classNode.enums) {
				...
				for (const child of IntelliSenseUtility.getChildrenOfClassByVersion(
					classNode,
					version,
				)) {
					if (child.enums) {
	 ...
	 * @param version target version
	 */
	static getRangeOfPropertyByVersion(
		propertyNode: PropertyNode,
		version: number,
	): ClassNode[] {
		if (!propertyNode.range) {
	 ...
	 * @param version target version
	 */
	static getChildrenOfClassByVersion(
		classNode: ClassNode,
		version: number,
	): ClassNode[] {
	 ...
	 * @param version target version
	 */
	static getPropertiesOfClassByVersion(
		classNode: ClassNode,
		version: number,
	): ClassNode[] {
	 ...
	 */
	static isAvailableByVersion(
		version: number,
		versionNode: VersionNode | undefined,
	): boolean {
		if (!versionNode) {
			return true;
		}
