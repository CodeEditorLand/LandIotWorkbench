	...
	implements vscode.CompletionItemProvider
{
	 ...
	 */
	private static completeTextForParse(
	...
	private static createCompletionItem(
		...
		const completionItem: vscode.CompletionItem = {
			...
			kind: suggestion.isProperty
				? vscode.CompletionItemKind.Property
				: vscode.CompletionItemKind.Value,
			insertText: new vscode.SnippetString(
				suggestion.insertText + separator,
			),
	...
	private static evaluateOverwriteRange(
		...
		if (
			...
			const word: string =
				DigitalTwinCompletionItemProvider.getCurrentWord(
					document,
					position,
				);
	 ...
	 * @param offset offset
	 */
	private static evaluateSeparatorAfter(
		text: string,
		offset: number,
	): string {
	...
	private static suggestProperty(
		...
		if (
			classNode &&
			!IntelliSenseUtility.isAvailableByVersion(
				version,
				classNode.version,
			)
		) {
		...
		if (!classNode) {
			...
			DigitalTwinCompletionItemProvider.suggestReservedProperty(
				includeValue,
				exist,
				required,
				suggestions,
	...
	provideCompletionItems(
		...
		if (node === parent.children[0]) {
			...
			DigitalTwinCompletionItemProvider.suggestValue(
				modelContent.version,
				parent,
				suggestions,
			);
