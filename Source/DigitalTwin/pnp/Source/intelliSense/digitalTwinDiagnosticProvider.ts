...
export class DigitalTwinDiagnosticProvider {
	...
	private static findClassNode(
		...
		if (propertyNode.range) {
			return propertyNode.range.find(
				(classNode) =>
					IntelliSenseUtility.getClassType(classNode) === type,
			);
		}
		return undefined;
	}
	...
	private static addProblemOfInvalidType(
		...
		const validTypes: string[] =
			IntelliSenseUtility.getValidTypes(digitalTwinNode);
		const message: string = [
			DiagnosticMessage.InvalidType,
			...validTypes,
	...
	private static validateObjectNode(
		...
		if (classNode.constraint && classNode.constraint.required) {
			const requiredProperties: string[] =
				classNode.constraint.required.filter((property) => {
					// property is not available in version
					if (
						!IntelliSenseUtility.isReservedName(property) &&
						!expect.has(property)
					) {
						return false;
					}
					...
					if (
						property === DigitalTwinConstants.CONTEXT &&
		...
		for (const child of jsonNode.children) {
			...
			switch (propertyName) {
				case DigitalTwinConstants.ID:
					...
					propertyNode = IntelliSenseUtility.getPropertyNode(
				...
				case DigitalTwinConstants.CONTEXT:
					...
					if (
						...
						classNode.constraint.required.includes(
							DigitalTwinConstants.CONTEXT,
						)
					) {
						if (
							IntelliSenseUtility.getDigitalTwinVersion(
								propertyPair.value,
							) !== version
						) {
							DigitalTwinDiagnosticProvider.addProblem(
	...
	private static validateArrayNode(
		...
		for (const child of jsonNode.children) {
			propertyPair =
				DigitalTwinDiagnosticProvider.getNamePropertyPair(child);
			if (propertyPair) {
	...
	private static validateStringNode(
		...
		const classNode: ClassNode | undefined =
			DigitalTwinDiagnosticProvider.findClassNode(
				digitalTwinNode,
				ValueSchema.String,
			);
		// validate enum node
	...
	private static validateEnumNode(
		...
		const enums: string[] = IntelliSenseUtility.getEnums(
			digitalTwinNode,
			version,
		);
		if (!enums.length) {
	...
	private provideDiagnostics(
		...
		const digitalTwinNode: PropertyNode | undefined =
